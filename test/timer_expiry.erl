...
-type server_timer() :: {chronos:server_name(), chronos:timer_name()}.
...
-record(state, {
    ...
    durations = [] :: [{unique_timer(), chronos:timer_duration()}],
    ...
    Reply =
        case
            chronos:start_timer(
                ...
                Timer,
                Duration,
                {timer_expiry, expire, [Server, Timer, Now]}
            )
        of
            ok ->
                Now;
