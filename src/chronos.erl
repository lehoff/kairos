...
-record(chronos_state, {running = [] :: [{timer_name(), reference(), callback()}]}).
...
-spec stop_timer(server_name(), timer_name()) ->
    {'ok', non_neg_integer()}
    | 'not_running'
    | {'error', term()}.
...
handle_call(
    {start_timer, Name, Time, Callback},
    _From,
    ...
    {reply, ok, State#chronos_state{running = [{Name, TRefNew, Callback} | R1]}};
handle_call(
    {stop_timer, Name},
    _From,
    #chronos_state{running = R} = State
    ...
    NewR =
        case lists:keytake(Timer, 1, R) of
            {value, {_, TRef, Callback}, R1} ->
                ...
                R1;
            %% has to ignore since TRef is not the current one
            {value, _, R1} ->
                R1;
            false ->
                R
